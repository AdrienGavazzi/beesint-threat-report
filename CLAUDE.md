...
- **`flex-wrap` + `gap` silently drops the gap once wrapping actually forces multiple lines** (confirmed empirically via a real render, not just CSS reading — distinct from the nested-flex-text-wrap issue above). `.kpi-grid`/`.chart-row` use `flex+gap` safely only because they never have enough cards to wrap past 1 row. `.lineage-grid` has 7 cards wrapping to 3 rows and rendered with literally zero gap (borders touching) until fixed by giving `.lineage-card` an explicit `margin` instead of relying on the parent's `gap`. Any new grid that might wrap to 2+ rows should use the margin pattern from the start, not `gap`.
